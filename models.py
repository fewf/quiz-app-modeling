...
# students in the below case is a collection of students who choose the response
# as the answer.
class QuizQuestionResponse(models.Model):
    quiz_question = models.ForeignKey(QuizQuestion)
    response = models.CharField(max_length=500)
    students = models.ManyToManyField(Student, through='StudentQuizQuestionResponse')
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)

# extra fields needed to keep track of when a quiz is submitted.
class StudentQuizQuestionResponse(models.Model):
